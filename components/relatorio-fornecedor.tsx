...
export function RelatorioFornecedor() {
  ...
  useEffect(() => {
    if (fornecedores.length > 0 && !fornecedorSelecionado) {
      setFornecedorSelecionado(fornecedores[0].id)
    }
  }, [fornecedores, fornecedorSelecionado])

  const fornecedorAtual = useMemo(() => {
    return fornecedores.find((f) => f.id === fornecedorSelecionado)
  }, [fornecedores, fornecedorSelecionado])

  const calcularTotalDivida = useMemo(() => {
    if (!fornecedorAtual) return 0
    return fornecedorAtual.pagamentos
          ...
          {fornecedorAtual ? (
            <div className="space-y-4">
              <div className="text-2xl font-bold">
                Total da Dívida: {calcularTotalDivida.toLocaleString("pt-MZ", { style: "currency", currency: "MZN" })}
              </div>
              <Table>
